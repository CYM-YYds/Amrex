...
- `amrex-26.06/`：上游版本对照；BOX3D 已采用该版本，其他算例是否迁移以各自 `config/GNUmakefile` 为准
...
- 本仓库已确认以 `CLAUDE.md` 为规则真身，`AGENTS.md` 必须是指向它的软链；Codex 通过该软链读取规则。运行 `neat-freak` 时不得再建议反转这一关系。
...
- BOX3D 当前使用 `amrex-26.06` 和 C++20，构建需要 GCC 11 或更新版本
