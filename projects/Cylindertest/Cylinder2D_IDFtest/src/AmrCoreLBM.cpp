...
#include <numeric>   // 用于 std::iota, std::partial_sum
...
void AmrCoreLBM::BuildActiveEulerSet(int lev) {
    ...
    if (idf_geometry_built && !idf_local_count_valid) {
        ...
        for (MyParIter pti(*mypc, lev); pti.isValid(); ++pti) {
            ...
            for (long i = 0; i < n; ++i) {
                ...
                // 方案 C: 利用粒子ID从1开始顺序分配的特性，直接计算全局索引
                int global_idx = pid - 1;
                if (global_idx >= 0 && global_idx < idf_NL_global) {
                    idf_local_lag_global_indices.push_back(global_idx);
                } else {
                    amrex::Print() << "[IDF][WARNING] particle ID " << pid
                                   << " out of range [1, " << idf_NL_global << "] after regrid!" << std::endl;
                }
            }
            idf_local_NL += n;
        }

        // 更新 idf_all_NL（使用 AMReX API: Gather + Bcast 实现 Allgather）
        int nprocs = ParallelDescriptor::NProcs();
        int root = ParallelDescriptor::IOProcessorNumber();
        idf_all_NL = ParallelDescriptor::Gather(idf_local_NL, root);
        ParallelDescriptor::Bcast(idf_all_NL.data(), nprocs, root);
    ...
    // ========== Step 2: MPI 汇总拉格朗日点位置和ID（使用 AMReX API） ==========
    // 收集每个进程的粒子数量（Gather + Bcast 实现 Allgather）
    int root = ParallelDescriptor::IOProcessorNumber();
    idf_all_NL = ParallelDescriptor::Gather(idf_local_NL, root);
    ParallelDescriptor::Bcast(idf_all_NL.data(), nprocs, root);
    ...
    ParallelDescriptor::Gatherv(local_lag_pos.data(), idf_local_NL * 2,
                                unsorted_lag_pos.data(), pos_recvcounts, pos_displs, root);
    ParallelDescriptor::Bcast(unsorted_lag_pos.data(), total_pos, root);
    ...
    ParallelDescriptor::Gatherv(local_lag_ids.data(), idf_local_NL,
                                all_lag_ids.data(), idf_all_NL, offsets, root);
    ParallelDescriptor::Bcast(all_lag_ids.data(), idf_NL_global, root);
    ...
    for (int i = 0; i < idf_local_NL; ++i) {
        ...
        if (global_idx >= 0 && global_idx < idf_NL_global) {
            ...
            amrex::Abort("IDF BuildActiveEulerSet particle ID out of range");
        }
    }
...
void AmrCoreLBM::IDF_InterpolateEulerToLag(int lev) {
    ...
    // Step 3: 汇总插值速度到全局向量（使用 AMReX API: Gatherv + Bcast）
    // 复用 BuildActiveEulerSet 中已更新的 idf_all_NL
    int nprocs = ParallelDescriptor::NProcs();
    int root = ParallelDescriptor::IOProcessorNumber();
    ...
    ParallelDescriptor::Gatherv(local_interp_ux.data(), idf_local_NL,
                                unsorted_interp_ux.data(), idf_all_NL, interp_displs, root);
    ParallelDescriptor::Bcast(unsorted_interp_ux.data(), idf_NL_global, root);
    ParallelDescriptor::Gatherv(local_interp_uy.data(), idf_local_NL,
                                unsorted_interp_uy.data(), idf_all_NL, interp_displs, root);
    ParallelDescriptor::Bcast(unsorted_interp_uy.data(), idf_NL_global, root);
    ParallelDescriptor::Gatherv(local_interp_rho.data(), idf_local_NL,
                                unsorted_interp_rho.data(), idf_all_NL, interp_displs, root);
    ParallelDescriptor::Bcast(unsorted_interp_rho.data(), idf_NL_global, root);
    ParallelDescriptor::Gatherv(local_interp_ids.data(), idf_local_NL,
                                all_interp_ids.data(), idf_all_NL, interp_displs, root);
    ParallelDescriptor::Bcast(all_interp_ids.data(), idf_NL_global, root);
