...
int main(int argc, char* argv[]) {
...

    {
        ...
        // fetch runtime params from AmrCoreLBM
        ...
        (void)regrid_int; // may be unused if regrid is disabled in this case
        amrex::Real cur_time = begin_step * dt_0;

        // 依据 begin_step 判断是否重启
        if (begin_step > 0) {
            lid.ReadCheckpoint();
            // 读入后再打印，以反映重启所加载的网格与参数
            lid.PrintMeshInfo();
            lid.PrintLbmParm();
