...
void denseMatVecWithScale(const std::vector<amrex::Real>& A,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int j = 0; j < N; ++j)
            acc += row[j] * x[j];
        y[i] = 2.0 * idf_interp_rho[i] / dt_min * acc;
    }
}
...
void AmrCoreLBM::BuildActiveEulerSet(int lev, int particle_idx, IDFData& idf_data) {
    auto& mypc = particles[particle_idx];
    // 仅处理第 particle_idx 个球体
    if (particles.size() <= static_cast<size_t>(particle_idx) || !mypc)
        return;

    // 静止边界优化：
    // 1. 矩阵 A^(-1) 只需构建一次（idf_data.idf_geometry_built）
    // 2. 本地粒子计数 idf_data.idf_local_NL 需要在每次 regrid 后更新（idf_data.idf_local_count_valid）

    // 如果几何已构建且本地计数有效，直接返回
    if (idf_data.idf_geometry_built && idf_data.idf_local_count_valid) {
        return;
    }
    ...
    if (idf_data.idf_geometry_built && !idf_data.idf_local_count_valid) {
        ...
        idf_data.idf_local_NL = 0;

        for (MyParIter pti(*mypc, lev); pti.isValid(); ++pti) {
            const long n = pti.numParticles();
            if (n == 0)
                continue;
            idf_data.idf_local_NL += n;
        }

        MPI_Allgather(&idf_data.idf_local_NL, 1, MPI_INT,
                      idf_data.idf_all_NL.data(), 1, MPI_INT,
                      ParallelDescriptor::Communicator());

        idf_data.idf_local_count_valid = true;
        return;
    }
    ...
    std::vector<Real> local_lag_pos;   // [x0, y0,z0, x1, y1, z1, ...]
    std::vector<int> local_lag_ids;    // 粒子全局ID（用于排序）
    std::set<IntVect> local_euler_set; // 本进程的活跃欧拉点

    // 使用 LagrangeParticleContainer 的接口收集粒子数据
    idf_data.idf_local_NL = mypc->IDF_CollectParticleData(lev, local_lag_pos, local_lag_ids);

    // 根据收集到的粒子位置计算活跃欧拉点
    for (int p = 0; p < idf_data.idf_local_NL; ++p) {
        Real xt_phys = local_lag_pos[3 * p];
        Real yt_phys = local_lag_pos[3 * p + 1];
        Real zt_phys = local_lag_pos[3 * p + 2];

        Real xt = xt_phys / delta;
        Real yt = yt_phys / delta;
        Real zt = zt_phys / delta;
        ...
        for (int x = -2; x <= 2; x++) {
            for (int y = -2; y <= 2; y++) {
                ...
                Real ly = yt - (yy + 0.5);
                Real lz = zt - (zz + 0.5);
                Real IB_Interp = delta3p(lx) * delta3p(ly) * delta3p(lz);
    ...
    idf_data.idf_all_NL.resize(nprocs);
    MPI_Allgather(&idf_data.idf_local_NL, 1, MPI_INT,
                  idf_data.idf_all_NL.data(), 1, MPI_INT,
                  ParallelDescriptor::Communicator());
    ...
    std::partial_sum(idf_data.idf_all_NL.begin(), idf_data.idf_all_NL.end(), offsets.begin() + 1);
    idf_data.idf_NL_global = offsets[nprocs];
    idf_data.idf_local_offset = offsets[myrank]; // 保留用于兼容性

    // ===== Step 2a & 2b: 汇总所有粒子位置（按粒子ID直接定位，无需排序） =====
    // 方案 C: 利用粒子ID从1开始顺序分配的特性
    // 全局索引 = 粒子ID - 1，无需汇总ID和排序

    // 粒子位置有 x,y,z 三个坐标，需要 ×3
    std::vector<int> pos_recvcounts(nprocs), pos_displs(nprocs + 1, 0);
    for (int i = 0; i < nprocs; ++i) {
        pos_recvcounts[i] = idf_data.idf_all_NL[i] * 3;
    }
    ...
    MPI_Allgatherv(local_lag_pos.data(), idf_data.idf_local_NL * 3, ParallelDescriptor::Mpi_typemap<Real>::type(), unsorted_lag_pos.data(), pos_recvcounts.data(), pos_displs.data(),
                   ParallelDescriptor::Mpi_typemap<Real>::type(),
                   ParallelDescriptor::Communicator());

    // ===== Step 2c: 按粒子ID直接重排位置数据（无需排序） =====
    // 粒子 ID 从 1 开始，全局索引 = ID - 1
    idf_data.idf_lag_pos_global.resize(idf_data.idf_NL_global * 3);

    // 遍历 unsorted 数据，根据粒子 ID 直接写入正确位置
    // 需要先收集粒子 ID 来定位
    std::vector<int> all_lag_ids(idf_data.idf_NL_global);
    // Allgatherv 汇总所有粒子 ID 到每个进程
    MPI_Allgatherv(local_lag_ids.data(), idf_data.idf_local_NL, MPI_INT,
                   all_lag_ids.data(), idf_data.idf_all_NL.data(), offsets.data(),
                   MPI_INT, ParallelDescriptor::Communicator());

    // 按粒子 ID 直接写入全局位置数组
    for (int i = 0; i < idf_data.idf_NL_global; ++i) {
        int pid = all_lag_ids[i];
        int global_idx = pid - 1; // 粒子ID从1开始
        if (global_idx >= 0 && global_idx < idf_data.idf_NL_global) {
            idf_data.idf_lag_pos_global[3 * global_idx + 0] = unsorted_lag_pos[3 * i + 0];
            idf_data.idf_lag_pos_global[3 * global_idx + 1] = unsorted_lag_pos[3 * i + 1];
            idf_data.idf_lag_pos_global[3 * global_idx + 2] = unsorted_lag_pos[3 * i + 2];
        }
    }
    ...
    idf_data.idf_active_euler_nodes_global.assign(global_euler_set.begin(), global_euler_set.end());
    idf_data.idf_NE_global = global_euler_set.size();

    // 构建全局欧拉点索引映射
    idf_data.idf_euler_index_map_global.clear();
    for (int i = 0; i < idf_data.idf_NE_global; ++i) {
        idf_data.idf_euler_index_map_global[idf_data.idf_active_euler_nodes_global[i]] = i;
    }

    // 仅在第一次调用时输出详细信息（使用particle_idx标记）
    static std::vector<bool> first_call(particle_num, true);
    if (particle_idx < static_cast<int>(first_call.size())) {
        if (first_call[particle_idx] && ParallelDescriptor::IOProcessor()) {
            amrex::Print() << "[IDF_3D] BuildActiveEulerSet(particle " << particle_idx
                           << "): NL_global=" << idf_data.idf_NL_global
                           << ", NE_global=" << idf_data.idf_NE_global << std::endl;
            first_call[particle_idx] = false;
        }
    }

    // 标记几何已构建（静止边界优化）
    idf_data.idf_geometry_built = true;
    idf_data.idf_local_count_valid = true;
}
...
void AmrCoreLBM::ApplyIDF(int lev) {
    ...
    // ApplyIDF第一次调用时初始化IDF数据结构
    if (!idf_data_initialized) {
        idf_data.resize(particle_num); // 为每个球体创建独立的IDFData实例
        idf_data_initialized = true;
        if (ParallelDescriptor::IOProcessor()) {
            amrex::Print() << "[IDF] 初始化了 " << particle_num << " 个球体的IDF数据结构" << std::endl;
        }
    }
    ...
    for (int p = 0; p < particle_num; p++) {
        ...
        IDFData& idf_data_p = idf_data[p];

        // Step 1: 插值欧拉速度到拉格朗日点（包括全局汇总）
        IDF_InterpolateEulerToLag(lev, p, idf_data_p);

        // Step 2: 组装该球体的全局矩阵 A = D_I × D_E
        IDF_AssembleMatrix(lev, p, idf_data_p);

        // Step 3: 求解线性系统得到拉格朗日力
        IDF_SolveSystem(lev, p, idf_data_p);

        // Step 4: 传播拉格朗日力到欧拉网格
        IDF_SpreadLagToEuler(lev, p, idf_data_p);
    }
...
void AmrCoreLBM::IDF_InterpolateEulerToLag(int lev, int particle_idx, IDFData& idf_data) {
    auto& mypc = particles[particle_idx];
    if (particles.size() <= static_cast<size_t>(particle_idx) || !mypc)
        return;
    ...
    // }
    // if (sum_n != idf_data.idf_local_NL) {
    //     amrex::Print() << "[IDF][ERROR] local particle count mismatch: sum_n=" << sum_n
    //                    << " idf_local_NL=" << idf_data.idf_local_NL << std::endl;
    ...
    // AMREX_ALWAYS_ASSERT_WITH_MESSAGE(static_cast<int>(local_interp_ux.size()) == idf_data.idf_local_NL,
    //                                  "IDF_3D local buffer size mismatch for particle " +
    //                                      std::to_string(particle_idx));

    // ========== Step 3: 汇总插值速度到全局向量 ==========
    std::vector<int> interp_displs(nprocs + 1, 0);
    std::partial_sum(all_NL.begin(), all_NL.end(), interp_displs.begin() + 1);
    ...
    std::vector<Real> unsorted_interp_ux(NL_global), unsorted_interp_uy(NL_global),
        unsorted_interp_uz(NL_global), unsorted_interp_rho(NL_global);
    std::vector<int> all_interp_ids(NL_global);

    MPI_Allgatherv(local_interp_ux.data(), local_NL, ParallelDescriptor::Mpi_typemap<Real>::type(),
                   unsorted_interp_ux.data(), all_NL.data(), interp_displs.data(),
                   ParallelDescriptor::Mpi_typemap<Real>::type(), ParallelDescriptor::Communicator());

    MPI_Allgatherv(local_interp_uy.data(), local_NL, ParallelDescriptor::Mpi_typemap<Real>::type(),
                   unsorted_interp_uy.data(), all_NL.data(), interp_displs.data(),
                   ParallelDescriptor::Mpi_typemap<Real>::type(), ParallelDescriptor::Communicator());

    MPI_Allgatherv(local_interp_uz.data(), local_NL, ParallelDescriptor::Mpi_typemap<Real>::type(),
                   unsorted_interp_uz.data(), all_NL.data(), interp_displs.data(),
                   ParallelDescriptor::Mpi_typemap<Real>::type(), ParallelDescriptor::Communicator());

    MPI_Allgatherv(local_interp_rho.data(), local_NL, ParallelDescriptor::Mpi_typemap<Real>::type(),
                   unsorted_interp_rho.data(), all_NL.data(), interp_displs.data(),
                   ParallelDescriptor::Mpi_typemap<Real>::type(), ParallelDescriptor::Communicator());

    MPI_Allgatherv(local_interp_ids.data(), local_NL, MPI_INT,
                   all_interp_ids.data(), all_NL.data(), interp_displs.data(),
                   MPI_INT, ParallelDescriptor::Communicator());

    // ========== Step 3b: 按粒子ID直接写入全局数组 ==========
    idf_data.idf_interp_u_x.resize(idf_data.idf_NL_global);
    idf_data.idf_interp_u_y.resize(idf_data.idf_NL_global);
    idf_data.idf_interp_u_z.resize(idf_data.idf_NL_global);
    idf_data.idf_interp_rho.resize(idf_data.idf_NL_global);
    for (int i = 0; i < idf_data.idf_NL_global; ++i) {
        int pid = all_interp_ids[i];
        int global_idx = pid - 1; // 粒子ID从1开始
        if (global_idx >= 0 && global_idx < NL_global) {
            idf_data.idf_interp_u_x[global_idx] = unsorted_interp_ux[i];
            idf_data.idf_interp_u_y[global_idx] = unsorted_interp_uy[i];
            idf_data.idf_interp_u_z[global_idx] = unsorted_interp_uz[i];
            idf_data.idf_interp_rho[global_idx] = unsorted_interp_rho[i];
        }
    }

    // ========== Step 4: 初始化目标速度和构建RHS ==========
    idf_data.idf_target_u_x.assign(idf_data.idf_NL_global, 0.0);
    idf_data.idf_target_u_y.assign(idf_data.idf_NL_global, 0.0);
    idf_data.idf_target_u_z.assign(idf_data.idf_NL_global, 0.0);

    // 仅在第一次调用时输出
    static std::vector<bool> first_interp(2, true);
    if (particle_idx < static_cast<int>(first_interp.size())) {
        if (first_interp[particle_idx] && ParallelDescriptor::IOProcessor()) {
            amrex::Print() << "[IDF_3D] IDF_InterpolateEulerToLag(particle " << particle_idx
                           << "): NL_global=" << idf_data.idf_NL_global << std::endl;
            first_interp[particle_idx] = false;
        }
    }
}

void AmrCoreLBM::IDF_AssembleMatrix(int lev, int particle_idx, IDFData& idf_data) {
    auto& mypc = particles[particle_idx];
    if (particles.size() <= static_cast<size_t>(particle_idx) || !mypc)
        return;

    // 如果矩阵已构建且大小正确，跳过
    if (idf_data.idf_matrix_built &&
        idf_data.idf_A.size() == static_cast<size_t>(idf_data.idf_NL_global * idf_data.idf_NL_global))
        return;
    ...
    int NL = idf_data.idf_NL_global;
    int NE = idf_data.idf_NE_global;
    ...
    auto const& emap = idf_data.idf_euler_index_map_global;

    // 基于全局拉格朗日点位置构建 D_I 和 D_E
    for (int p = 0; p < NL; ++p) {
        Real xp = idf_data.idf_lag_pos_global[3 * p];
        Real yp = idf_data.idf_lag_pos_global[3 * p + 1];
        Real zp = idf_data.idf_lag_pos_global[3 * p + 2];
        Real xt = xp / delta;
        Real yt = yp / delta;
        Real zt = zp / delta;
        ...
        for (int z = -2; z <= 2; ++z) {
            for (int y = -2; y <= 2; ++y) {
                for (int x = -2; x <= 2; ++x) {
                    ...
                    Real wE = wI; // 系数后续计算再补充
                    // Real wE = wI * IB_weight;

                    if (std::abs(wI) > 1e-15) {
                        idf_data.DI_rows[p].emplace_back(eidx, wI);
                    }
                    if (std::abs(wE) > 1e-15) {
                        idf_data.DE_cols[p].emplace_back(eidx, wE);
                    }
                }
            }
        }
        ...
        idf_data.idf_A.assign(static_cast<size_t>(NL) * NL, 0.0);
        ...
        for (int i = 0; i < NL; ++i) {
            for (auto const& ie : DI_rows[i]) {
                ...
                for (auto const& jw : DE_by_euler[eidx]) {
                    int j = jw.first;
                    Real wE = jw.second;
                    idf_data.idf_A[i * NL + j] += wI * wE;
                }
            }
        }

        idf_data.idf_matrix_built = true;

        /*     if (ParallelDescriptor::IOProcessor()) {
                Real maxA = 0.0, sumDiag = 0.0;
                for (int i = 0; i < NL; ++i) {
                    sumDiag += idf_data.idf_A[i * NL + i];
                    for (int j = 0; j < NL; ++j) {
                        maxA = std::max(maxA, std::abs(idf_data.idf_A[i * NL + j]));
                    }
                }
                ...
                if (matrixFile.is_open()) {
                    ...
                    for (int i = 0; i < NL; ++i) {
                        for (int j = 0; j < NL; ++j) {
                            matrixFile << idf_data.idf_A[i * NL + j];
                            if (j < NL - 1)
        ...
        // ======== 静止边界优化：预计算 A 的逆矩阵 ========
        if (!idf_data.idf_inverse_built) {
            if (ParallelDescriptor::IOProcessor()) {
                amrex::Print() << "[IDF] Computing A^(-1) for static boundary..." << std::endl;
            }

            bool success = computeMatrixInverse(idf_data.idf_A, idf_data.idf_A_inv, NL);

            if (success) {
                idf_data.idf_inverse_built = true;
                if (ParallelDescriptor::IOProcessor()) {
                    Real maxAinv = 0.0;
                    for (int i = 0; i < NL * NL; ++i) {
                        maxAinv = std::max(maxAinv, std::abs(idf_data.idf_A_inv[i]));
                    }
    ...
    // 求解线性系统得到拉格朗日力
    void AmrCoreLBM::IDF_SolveSystem(int lev, int particle_idx, IDFData& idf_data) {
        int NL = idf_data.idf_NL_global;
        if (NL == 0)
            return;
        // ====== Step 1: 使用插值后的密度 ======
        ...
        idf_data.idf_rhs_x.resize(NL);
        idf_data.idf_rhs_y.resize(NL);
        for (int i = 0; i < NL; ++i) {
            idf_data.idf_rhs_x[i] = idf_data.idf_interp_u_x[i] - idf_data.idf_target_u_x[i];
            idf_data.idf_rhs_y[i] = idf_data.idf_interp_u_y[i] - idf_data.idf_target_u_y[i];
            idf_data.idf_rhs_z[i] = idf_data.idf_interp_u_z[i] - idf_data.idf_target_u_z[i];
        }

        // ====== Step 3: 求解线性系统 A * f_vel = rhs 并同时计算力密度 ======
        idf_data.idf_sol_x.resize(NL);
        idf_data.idf_sol_y.resize(NL);
        idf_data.idf_sol_z.resize(NL);

        if (idf_data.idf_inverse_built) {
            // 使用预计算的 A^(-1)：矩阵-向量乘法并同时应用密度系数
            // idf_sol = rhot_coeff * (A^(-1) * rhs)
            denseMatVecWithScale(idf_data.idf_A_inv, idf_data.idf_rhs_x, idf_data.idf_interp_rho, idf_data.idf_sol_x);
            denseMatVecWithScale(idf_data.idf_A_inv, idf_data.idf_rhs_y, idf_data.idf_interp_rho, idf_data.idf_sol_y);
            denseMatVecWithScale(idf_data.idf_A_inv, idf_data.idf_rhs_z, idf_data.idf_interp_rho, idf_data.idf_sol_z);
        ...
        mypc->IDF_WriteForceToParticles(lev, idf_data.idf_sol_x, idf_data.idf_sol_y, idf_data.idf_sol_z, idf_data.idf_NL_global);
    ...
    // 传播拉格朗日力到欧拉网格
    void AmrCoreLBM::IDF_SpreadLagToEuler(int lev, int particle_idx, IDFData& idf_data) {
        auto& mypc = particles[particle_idx];
        ...
        // 仅在第一次调用时输出
        static std::vector<bool> first_spread(particle_num, true);
        if (particle_idx < static_cast<int>(first_spread.size())) {
            if (first_spread[particle_idx] && ParallelDescriptor::IOProcessor()) {
                amrex::Print() << "[IDF_3D] IDF_SpreadLagToEuler(particle " << particle_idx
                               << ") done." << std::endl;
                first_spread[particle_idx] = false;
            }
        }
    }
