...
#DSUB -n box3d_interp_norm
#DSUB -A root.huazkjdxmrsgjzdsyshi
#DSUB -q root.default
#DSUB -R cpu=32;mem=49152;gpu=1
#DSUB -N 1
#DSUB -o logs/submit/%J-interp-norm.log
#DSUB -e logs/submit/%J-interp-norm.log
#DSUB -l cuda122
...
hostfile="$(mktemp /tmp/box3d-interp-norm.XXXXXX)"
...
reference_prefix="ddf_norm_interp_mode0_"
...
checkpoint.write_particles=0 lbm.interp_mode=0"

run_case \
    "export CUDA_VISIBLE_DEVICES=0; exec ./main3d.gnu.TPROF.MPI.CUDA.ex config/inputs \
max_step=64 amr.plot_int=1000000 checkpoint.chk_int=-1 \
checkpoint.write_particles=0 lbm.interp_mode=1 \
