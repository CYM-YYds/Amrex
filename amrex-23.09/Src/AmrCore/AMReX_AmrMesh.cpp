...
namespace amrex
{
    ...
    AmrMesh::MakeNewGrids(int lbase, Real time, int &new_finest, Vector<BoxArray> &new_grids)
    {
        ...
        if (new_grids.size() < max_crse + 2)
        {
            new_grids.resize(max_crse + 2);
        }
    ...
    std::ostream &operator<<(std::ostream &os, AmrMesh const &amr_mesh)
    {
        ...
        os << "  ref_ratio =";
        for (int lev = 0; lev < amr_mesh.max_level; ++lev)
        {
            os << " " << amr_mesh.ref_ratio[lev];
        }
        os << "\n";
        os << "  blocking_factor =";
        for (int lev = 0; lev <= amr_mesh.max_level; ++lev)
        {
            os << " " << amr_mesh.blocking_factor[lev];
        }
        os << "\n";
        os << "  max_grid_size =";
        for (int lev = 0; lev <= amr_mesh.max_level; ++lev)
        {
            os << " " << amr_mesh.max_grid_size[lev];
        }
        os << "\n";
        os << "  n_error_buf =";
        for (int lev = 0; lev < amr_mesh.max_level; ++lev)
        {
            os << " " << amr_mesh.n_error_buf[lev];
        }
        os << "\n";
